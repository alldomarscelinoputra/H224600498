...
function tampilkanData($conn) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        echo "<table border='1'>";
        echo "<tr><th>ID</th><th>NIM</th><th>Nama</th><th>Alamat</th><th>Email</th><th>Status</th></tr>";

        while ($row = $result->fetch_assoc()) {
            echo "<tr><td>".$row["id"]."</td><td>".$row["nim"]."</td><td>".$row["nama"]."</td><td>".$row["alamat"]."</td><td>".$row["email"]."</td><td>".$status."</td></tr>";
        }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["tambah"])) {
        ...
        if (!empty($nim) && !empty($nama) && !empty($alamat) && !empty($email)) {
            ...
            $query = "INSERT INTO bukutamu(nim, nama, alamat, email) VALUES ('$nim', '$nama', '$alamat', '$email')";
            $result = $conn->query($query);
        ...
        $delete_query = "DELETE FROM bukutamu WHERE id=$id_to_delete";
...
<head>
    ...
    color: #fff; /* Tambahkan warna teks default */
}

.container {
    margin: 20px auto; /* Pusatkan konten */
    max-width: 850px; /* Lebarkan container */
    padding: 20px;
    background-color:#101010; /* Tambahkan latar belakang putih */
